...
public final class Rig {
    ...
    public func applyStageAnchor(_ states: [String: FixtureState]) -> [String: FixtureState] {
        let s = stage
        if s.stretch == 1.0 && !s.invertTilt && s.tiltFlipFixtures.isEmpty { return states }   // identity — skip the work on the default rig
        ...
        for f in fixtures where f.profile.id == "spiider_mode3" || f.profile.id == "t1_mode3" {
            ...
            if s.invertTilt { tilt = 1 - tilt }   // whole rig hangs inverted: flip beam from upstage onto the piano up front
            // Per-fixture extra flip for a mirror-hung fixture whose tilt axis runs OPPOSITE to its
            // partner (the two T1s are hung facing each other, so the global invert lands one on the
            // piano and throws the other back onto the cyc/screen). Flip just the odd one out.
            if s.tiltFlipFixtures.contains(f.name) { tilt = 1 - tilt }
            st.tilt = tilt
            out[f.name] = st
        }
        return out
